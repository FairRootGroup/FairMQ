...
/// @brief Memory allocators and interfaces related to managing memory via the trasport layer
///
...
/// All FairMQ related memory resources need to inherit from this interface class for the
/// getMessage() api.
class FairMQMemoryResource : public boost::container::pmr::memory_resource
{
  public:
    /// return the message containing data associated with the pointer (to start of
    /// buffer), e.g. pointer returned by std::vector::data() return nullptr if returning
...
/// This is the allocator that interfaces to FairMQ memory management. All allocations are
/// delegated to FairMQ so standard (e.g. STL) containers can construct their stuff in
...
class ChannelResource : public FairMQMemoryResource
{
  protected:
    ...
    // TODO: for now a map to keep track of allocations, something else would probably be
...
/// In combination with the ByteSpectatorAllocator this is an alternative to using span, as raw
/// memory (e.g. an existing buffer message) will be accessible with appropriate container.
...
// This in general (as in STL) is a bad idea, but here it is safe to inherit from an allocator since
// we have no additional data and only override some methods so we don't get into slicing and other
// problems.
template<typename T>
class SpectatorAllocator : public boost::container::pmr::polymorphic_allocator<T>
{
  public:
    ...
    // this is important for two reasons: one: it allows us to adopt an existing buffer (e.g.
    // incoming message) and quickly construct large vectors while skipping the element
    // initialization.
    template<class U>
    void construct(U*)
    {}
...
// return the message associated with the container or nullptr if it does not make sense (e.g. when
// we are just watching an existing message or when the container is not using FairMQMemoryResource
// as backend).
template<typename ContainerT>
// typename std::enable_if<
//    std::is_base_of<
//        boost::container::pmr::polymorphic_allocator<typename ContainerT::value_type>,
...
FairMQMessagePtr getMessage(ContainerT&& container_, FairMQMemoryResource* targetResource = nullptr)
{
    ...
    if ((!targetResource && resource)
        ...
        std::memcpy(
            static_cast<o2::byte*>(message->GetData()), container.data(), containerSizeBytes);
        return std::move(message);
    }
};

/// Return a vector of const ElemT, resource must be kept alive throughout the lifetime of the
...
/// Return a vector of const ElemT, takes ownership of the message, needs an upstream global
...
namespace internal {
/// A (thread local) singleton placeholder for the channel allocators. There will normally be 1-2
/// elements in the map.
// Ideally the transport class itself would hold (or be) the allocator, if that ever happens, this
// can go away.
