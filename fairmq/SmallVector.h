...
#ifndef FAIR_LLVM_COPY_ADT_SMALLVECTOR_H
#define FAIR_LLVM_COPY_ADT_SMALLVECTOR_H
...
protected:
  ...
  void assertSafeToReferenceAfterResize(const void *Elt [[maybe_unused]], size_t NewSize [[maybe_unused]]) {
...
extern template class llvm_copy::SmallVectorBase<uint32_t>;
#if SIZE_MAX > UINT32_MAX
extern template class llvm_copy::SmallVectorBase<uint64_t>;
#endif
...
namespace std {
  ...
  template<typename T>
  inline void
  swap(fair::llvm_copy::SmallVectorImpl<T> &LHS, fair::llvm_copy::SmallVectorImpl<T> &RHS) {
    LHS.swap(RHS);
  }
  ...
  template<typename T, unsigned N>
  inline void
  swap(fair::llvm_copy::SmallVector<T, N> &LHS, fair::llvm_copy::SmallVector<T, N> &RHS) {
    LHS.swap(RHS);
  }
